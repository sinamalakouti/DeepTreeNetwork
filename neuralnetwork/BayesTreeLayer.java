 ...
 * @author Adam Gibson
 */
public  class BayesTreeLayer extends BaseLayer<CustomLayer> {
    ...
    public BayesTreeLayer(NeuralNetConfiguration conf, Instances train, Instances test) {
        super(conf);
      
    }

    public BayesTreeLayer(NeuralNetConfiguration conf, INDArray input, Instances train, Instances test) {
...
//    TODO check and implement this function properly
    ...
    public Pair<Gradient, INDArray> backpropGradient(INDArray epsilon, LayerWorkspaceMgr workspaceMgr) {
        ...
        INDArray z = backpropOutput(epsilon, workspaceMgr); //Note: using preOutput(INDArray) can't be used as this does a setInput(input) and resets the 'appliedDropout' flag
...
//        		z.assign(input.mulRowVector(weight.transpose())); 
             ...
             if( neuron == 0) {
//              ret = layerConf().getActivationFn().getActivation(z, training);
            	 ret = activationModels.get(neuron).backprop(z, epsilon).getFirst();
            	 result = ret.dup().transpose();
             }else {
