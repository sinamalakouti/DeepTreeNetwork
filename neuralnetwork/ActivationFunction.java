...
//		in  = org.nd4j.linalg.ops.transforms.Transforms.sigmoid(in, false);
		...
		double [][] result = this.getActivationFunResult(in, arg1);
		in = Nd4j.create(result);
		return in;
	}


	private double [][] getActivationFunResult (INDArray arg0 , boolean arg1){
		...
		if (activationModel == null) {
			...
			try {
				activationModel.updateClassifier(instances);
			} catch (WekaException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (Exception e) {
	...
	public org.nd4j.linalg.primitives.Pair<INDArray, INDArray> backprop(INDArray arg0, INDArray arg1) {
		double [][] result = this.getActivationFunResult(arg0, true);
